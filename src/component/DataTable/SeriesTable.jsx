  ...
  const deleteid = async (id) => {
    ...
    try {
      ...
      if (error.response.data.message.field_name === "book_fk_2 (index)") {
        ...
        // );
        Swal.fire({
          text: "Series cannot be deleted at this time as it is currently in use.",
          icon: "warning",
        });
      }
    }
  ...
  return (
    <div className="flex flex-col gap-5 sm:flex-row  sm:justify-between">
        ...
        <TableContainer component={Paper}>
          <Table aria-label="simple table">
            <TableHead className="!bg-slate-400 !w-full">
              <TableRow>
                {["Subject", "Series", "Active", "Action"].map((header, i) => (
      ...
      <div className="flex w-full md:w-[60%] p-4">
        <AddSeries fetch={fetch} fetchdata={fetchdata} />
      </div>
    </div>
  );
}
