...
export default function SubjectTable() {
  ...
  const deleteid = async (id) => {
    ...
    try {
      ...
      if (
        error.response.data.message.field_name === "book_fk_3 (index)" ||
        error.response.data.message.field_name === "seriesmaster_fk (index)"
      ) {
        // alert(
        //   "The subject cannot be deleted at this time as it is currently in use."
        // );
        Swal.fire({
          text: "Subject cannot be deleted at this time as it is currently in use.",
          icon: "warning",
        });
      }
    }
  ...
  return (
    <div className="flex flex-col gap-5 sm:flex-row sm:gap-5 ">
        ...
        <TableContainer component={Paper}>
          <Table aria-label="simple table">
            <TableHead className="!bg-slate-400 !w-full">
              <TableRow>
                {["Subject", "Active", "Action"].map((header, i) => (
      ...
      <div className="flex w-full md:w-[60%] p-4">
        <AddSubject fetch={fetch} fetchdata={fetchdata} />
      </div>
    </div>
  );
}
